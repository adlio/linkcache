...
fn test_firefox_with_profile_dir() {
    linkcache::testutils::init_firefox_test_env();
    ...
    assert!(
        browser.places_path().exists(),
        "Test places.sqlite should exist"
    );
}
...
fn test_firefox_all_history() {
    ...
    assert!(
        titles.contains(&"Example Domain".to_string()),
        "Should have Example Domain history entry"
    );
    assert!(
        titles.contains(&"Wikipedia".to_string()),
        "Should have Wikipedia history entry"
    );
}
